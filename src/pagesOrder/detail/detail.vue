...
import { deleteMemberOrderApi, getMemberOrderByIdApi, getMemberOrderCancelByIdApi, getMemberOrderConsignmentByIdApi, getMemberOrderLogisticsByIdApi, getPayMockApi, getPayWxPayMiniPayApi, putMemberOrderReceiptByIdApi } from '@/services/order';
...
const getMemberOrderByIdData = async () => {
  ...
  if([OrderState.DaiShouHuo,OrderState.DaiPingJia,OrderState.YiWanCheng].includes(orderDetail.value.orderState)){
    //获取订单物流信息 仅在订单状态为待收货，待评价，已完成时，可获取物流信息
...
const onOrderPay= async()=>{
  if(import.meta.env.DEV){
    ...
    // #endif
    // #ifdef H5
    uni.showToast({ title: '待开发', icon: 'none' })
    // #endif
  }
...
const onCancelOrder=async()=>{
  if(!reason.value){
    uni.showToast({ title: '请选择取消订单的原因', icon: 'none' })
    return
  }
    ...
    <template v-if="isLoaded">
      ...
      <view class="overview" :style="{ paddingTop: safeAreaInsets!.top + 20 + 'px' }">
        ...
        <template v-else>
          ...
          <view class="button-group">
            ...
            <view v-if="isDev&&orderDetail!.orderState===OrderState.DaiFaHuo" @tap="onOrderSend" class="button"> 模拟发货 </view>
          <!-- 待收货状态: 展示确认收货按钮 -->
          <view v-if="orderDetail?.orderState === OrderState.DaiShouHuo" @tap="onOrderConfirm" class="button">
            确认收货
          </view>
          </view>
        </template>
      ...
      <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
        ...
        <template v-else>
          ...
          <!-- 待收货状态: 展示确认收货 -->
          <view class="button primary" v-if="orderDetail?.orderState===OrderState.DaiShouHuo" @tap="onOrderConfirm"> 确认收货 </view>
          <!-- 待评价状态: 展示去评价 -->
          <view class="button" v-if="orderDetail?.orderState===OrderState.DaiPingJia"> 去评价 </view>
          <!-- 待评价/已完成/已取消 状态: 展示删除订单 -->
          <view class="button delete" v-if="orderDetail!.orderState>=OrderState.DaiPingJia" @tap="onDeleteOrder"> 删除订单 </view>
        </template>
      </view>
    </template>
    <template v-else>
